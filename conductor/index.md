# AKRA Project Master Index

This is the central registry for all project tracks and implementation plans. Use this index to track active development and historical milestones.

## 🛠️ Project Status Legend
- 🟢 **Done**: Verified and merged.
- 🟡 **In Progress**: Active development.
- ⚪ **Pending**: Queued for execution.
- 🔴 **Blocked**: Waiting on external factors.

---

## 🏗️ Infrastructure & Foundation
- 🟢 **Authentication (AuthGuard)**: Standardized across all apps.
- 🟢 **API Layer (AKRA_API)**: Unified Supabase client.
- ⚪ **Git Sync & Audit**: [Plan: sync_github.md](./sync_github.md)
- ⚪ **Portal Upgrade**: [Plan: portal_upgrade_plan.md](./portal_upgrade_plan.md)

## 📱 Application Overhauls (Logic & Features)
- 🟢 **GR (Goods Receipt)**: [Archive: GR_overhaul_plan.md](./archive/GR_overhaul_plan.md)
- 🟢 **PR (Purchase Request)**: [Archive: PR_overhaul_plan.md](./archive/PR_overhaul_plan.md)
- 🟡 **PO (Purchase Order)**: [Plan: PO_overhaul_plan.md](./PO_overhaul_plan.md) (UI Sync done, Logic TBD)
- ⚪ **Return & Claim**: [Plan: Return_overhaul_plan.md](./Return_overhaul_plan.md)
- 🟢 **W5 Inventory**: [Archive: W5_overhaul_plan.md](./archive/W5_overhaul_plan.md)
- 🟢 **Transfers**: [Archive: Transfers_overhaul_plan.md](./archive/Transfers_overhaul_plan.md)

## 🎨 UI/UX Gold Standard (Sprint Complete)
- 🟢 **Global UI Sync**: [Archive: UI_UX_refinement_plan.md](./archive/UI_UX_refinement_plan.md)
- 🟢 **All App Refinements**: [Browse Archive Folder](./archive/)

---

## 📖 Key Documentation
- **Current State**: [PROJECT_STATE.md](../PROJECT_STATE.md)
- **Standard Workflow**: [WORKFLOW.md](../docs/WORKFLOW.md)
- **Technical Ledger**: [applied_solutions.md](../docs/applied_solutions.md)
